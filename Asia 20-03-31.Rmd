---
title: "Asian COVID19 Cases and Deaths; Rate of doubling."
...
df_g <- df_100[df_100$location == "Asia",] %>% 
...
g_cases_separate <- g_cases_separate + 
  ...
  ggtitle("Asia 29 March 2020", subtitle = "Author: JW Pickering; Data: John Hopkins University")

number_rows = ceiling(length(unique(df_g$country))/3)

ggsave(g_cases_separate, filename = "20-03-29 Asia Cases.jpg", width = 210  , height = 20 + 40 * number_rows, units = "mm")
(g_cases_separate)
```
...
df_g <- df_10[df_10$location == "Asia",] %>% 
  filter(!is.na(country))
# Correct one very odd value
...
g_deaths_separate <- ggplot() +
  ...
  coord_cartesian(ylim = c(0.25,1000)) +
  theme(legend.position = "none",
        panel.background = element_rect(fill = "seashell2"),
        panel.grid = element_line(colour = "grey80"),
        panel.grid.minor = element_blank()) 
...
ggsave(g_deaths_separate, filename = "20-03-29 Asia Deaths.jpg", width = 210  , height = 20 + 40 * number_rows , units = "mm")
(g_deaths_separate)
```
